...
export const fetchIsFavorite = async (id: string) => {
  const client = await getClient();
  const { data } = await client("/favorite/is-fav?collectionId=" + id);
  return data.result;
};

export const useIsFavorite = (id: string) => {
  const { data, isLoading, error, isSuccess } = useQuery(["isFavorites", id], {
